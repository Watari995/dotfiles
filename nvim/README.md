...
| [バッファ / ジャンプ移動](#バッファ--ジャンプ移動) | `<C-o>`, `<C-i>`, `<C-6>` |
...
| `<C-b>` | 一覧を上にスクロール（Explorer内） |
| `<C-f>` | 一覧を下にスクロール（Explorer内） |
...
| キー | 動作 |
|------|------|
| `<C-o>` | ジャンプリストを1つ前へ戻る（ファイルをまたいだ移動も含む） |
| `<C-i>` | ジャンプリストを1つ先へ進む |
| `<C-6>` (`<C-^>` と同じ) | 直前に編集していたバッファ (alternate file) とトグル |

`<C-o>` / `<C-i>` はカーソル位置のジャンプ履歴（`gg`, `G`, 検索, `:e` でのファイル切り替え, タグジャンプなど）を辿るため、別ファイルへ移動した後でも前のファイル・位置に戻れる。単純な `hjkl` はジャンプとして記録されない。履歴一覧は `:jumps` で確認できる。

`<C-6>` は現在のバッファと alternate file（直前に編集していたバッファ）を交互にトグルするだけのシンプルな切り替え。`:b#` でも同じ動作。
...
| `<leader>yp` | リポジトリ名を含むパスをクリップボードにコピー |

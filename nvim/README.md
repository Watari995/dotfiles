...
| [ユーティリティ](#ユーティリティ) | `<leader>cs`, `<leader>co`, `<leader>yp` |
...
| `gD` | 定義へジャンプ（2枚目のsplitで開く。なければ右に作成） |
| `gd` | 定義へジャンプ |
...
`K` は 1 回押すとカーソル下のシンボルの hover を参照します。hover 表示中にもう一度 `K` を押すと hover 内にフォーカスが移ります。エディタへ戻るには `<C-w>p` または `<C-w>w` を押します。

Go の interface メソッドから実装へ移動したい場合は、メソッド名にカーソルを置いて `gi` を押します。mock と本実装など複数候補がある場合は Snacks Picker で選択します。
...
| `<leader>wa` | カーソル下の単語を cspell 辞書へ追加 |
...
| キー | 動作 |
|------|------|
| `<leader>cs` | Claude Code を権限確認スキップで起動 |
| `<leader>co` | Codex を yolo モードで起動 |
